...
class Water:
    ...
    def __init__(self, temperature, pressure, state,
                 enthalpy=None):
        r'''
...
class LiBr_solution:
    ...
    def isobaric_heat_capacity(self, solution_temperature, method="Ren"):
        ...
        if method == 'Brak-Cyklis':
            ...
            for n in N:
                isobaric_heat_capacity += (A_n[n] * x**n) + (2 * B_n[n] * x**n * t)
...
def calculate_cold_temperature_out(Q, UA,
    ...
    print(hot_temperature_in, hot_temperature_out, cold_temperature_in)
    print(theta_2)
    print(solutions)
    exit()
    for i in solutions:
...
class Generator:
    ...
    def strong_solution_temp_out_LMTD(self, Q, HW_temp_in, HW_temp_out, WS_temp_in):
    ...
    def heat_generator_Ren(self, m_ref, m_ws, x_ws, x_ss, Tss_out, Tws_in, Tcon, Tref_out):
        ...
        Q_ref = m_ref * (Xbar_g * h_con + specific_heat_water_vapor * (Tref_out - Tss_out))
...
class Condenser:
    ...
    def refrigerant_temp_out_LMTD(self, Q, Tref_in, Tcw_in, Tcw_out):
        return calculate_hot_temperature_out(Q, self.UA, Tref_in, Tcw_in, Tcw_out)
...
class Evaporator:
    ...
    def refrigerant_temp_out_LMTD(self, Q, Tref_in, Tch_in, Tch_out):
        return calculate_cold_temperature_out(Q, self.UA, Tch_in, Tch_out, Tref_in)
...
class Absorber:
    ...
    def heat_absorber_Ren(self, m_ref, m_ws, x_ws, x_ss, Tss_in, Tws_out, Tcon, Tref_in):
        WeakSolution = LiBr_solution(x_ws)
        StrongSolution = LiBr_solution(x_ss)
        ...
        Q_ref = m_ref * (Xbar_g * h_con + specific_heat_water_vapor * (Tref_in - Tss_in) + cp_ss * (Tss_in - Tss_eq))
...
def Rens_algorithm(
    ...
    StrongSolution = LiBr_solution(0.60)
    WeakSolution = LiBr_solution(0.57)
    ...
    while error_10 > error_threshold:
        ...
        while error_8 > error_threshold:
            ...
            while error_4 > error_threshold:
                ...
                StrongSolution.mass_fraction = StrongSolution.Duhring_equilibrium_concentration(p8.temperature, p4.temperature)

                p6.temperature_eq = StrongSolution.Duhring_equilibrium_temperature(p10.temperature)
                
                # Assume value of T5
                p5.temperature = 50.6
                error_5 = 1
                while error_5 > error_threshold:
                    cp_ss = StrongSolution.isobaric_heat_capacity(np.mean([p5.temperature, p4.temperature]))

                    # Assume value of T1
                    p1.temperature = 38
                    error_1 = 1
                    while error_1 > error_threshold:
                        WeakSolution.mass_fraction = WeakSolution.Duhring_equilibrium_concentration(p10.temperature, p1.temperature)

                        CR = calc_circulation_ratio(StrongSolution.mass_fraction, WeakSolution.mass_fraction)

                        m_r = m_ss / CR
                        m_ws = m_ss + m_r

                        p3.temperature_eq = WeakSolution.Duhring_equilibrium_temperature(p8.temperature)
                        ...
                        # Solve for T14
                        p14.temperature = Absorber_.cooling_water_temp_out(Q_a, m_a, p13.temperature)
                        
                        # Calculate new T1 according to Qa = UA LMTD
                        T1_new = Absorber_.solution_temperature_out(Q_a, p5.temperature, p13.temperature, p14.temperature)
                    ...
                    # Calculate QSHE
                    Q_shx = SHX_.heat_flow_Ren(m_ss, p4.temperature, p5.temperature, StrongSolution.mass_fraction)

                    # Solve for T3
                    p3.temperature = SHX_.weak_solution_temperature_out_Ren(m_ws, WeakSolution.mass_fraction, p2.temperature, Q_shx)
                    
                    # Calculate new T5
                    T5_new = SHX_.LMTD_strong_solution_temperature_out(Q_shx, p4.temperature, p2.temperature, p3.temperature)
                ...
                # Solve for T12
                p12.temperature = Generator_.hot_water_temp_out(Q_g, m_g, p11.temperature)

                # Determinte new T4
                T4_new = Generator_.strong_solution_temp_out_LMTD(Q_g, p11.temperature, p12.temperature, p3.temperature)
            ...
            # Condenser
            # Calculate Qc
            Q_c = Condenser_.heat_condenser_Ren(m_r, p7.temperature, p8.temperature)
            
            # Solve for T16
            p16.temperature = Condenser_.cooling_water_temp_out(Q_c, m_cw, p15.temperature)

            # Determine new T8
            T8_new = Condenser_.refrigerant_temp_out_LMTD(Q_c, p7.temperature, p15.temperature, p16.temperature)
        ...
        # Solve for T18
        p18.temperature = Evaporator_.chilled_water_T_out(m_e, Q_e, p17.temperature)

        # Calculate new T10
        T10_new = Evaporator_.refrigerant_temp_out_LMTD(Q_e, p9.temperature, p17.temperature, p18.temperature)
